            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <div class="grid md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                        <h3 class="text-xl font-semibold text-yellow-800 mb-2">Bins Needing Attention</h3>
                        <p class="text-3xl font-bold text-yellow-600" id="bins-needing-attention">
                            {{ $binsNeedingAttention }}
                        </p>
                    </div>
                    <div class="bg-orange-50 border border-orange-200 rounded-lg p-6">
                        <h3 class="text-xl font-semibold text-orange-800 mb-2">Total Alerts</h3>
                        <p class="text-3xl font-bold text-orange-600" id="total-alerts">{{ $totalAlerts }}</p>
                    </div>
                    <div class="bg-green-50 border border-green-200 rounded-lg p-6">
                        <h3 class="text-xl font-semibold text-green-800 mb-2">Total Collections</h3>
                        <p class="text-3xl font-bold text-green-600" id="total-collections">{{ $totalCollections }}</p>
                    </div>
                </div>
                ...
                @if($bins->count() > 0)
                <div class="mb-8">
                    <h2 class="text-2xl font-bold text-gray-900 mb-4">Bins Needing Attention</h2>
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form id="edit-bin-form">
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="edit-bin-latitude">Latitude</label>
                    <input type="number" step="any" id="edit-bin-latitude" name="latitude"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="edit-bin-longitude">Longitude</label>
                    <input type="number" step="any" id="edit-bin-longitude" name="longitude"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                </div>
                <div class="mb-4">
...
function editBin(binId) {
    ...
    fetch(`/collector/bins/${binId}`, {
        ...
        .then(data => {
            ...
            document.getElementById('edit-bin-latitude').value = data.latitude;
            document.getElementById('edit-bin-longitude').value = data.longitude;
...
function addBinToTable(bin) {
    ...
    row.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${bin.name}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${bin.latitude}, ${bin.longitude}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${bin.level.charAt(0).toUpperCase() + bin.level.slice(1)}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
...
function updateBinInTable(binId, bin) {
    ...
    if (row) {
        ...
        row.cells[2].textContent = `${bin.latitude}, ${bin.longitude}`;
        row.cells[3].textContent = bin.level.charAt(0).toUpperCase() + bin.level.slice(1);
    }
}
